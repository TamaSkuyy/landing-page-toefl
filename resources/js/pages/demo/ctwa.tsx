...
/* ============================================================
   Full Bright Indonesia — TOEFL ITP landing page (PROJECT_MODE=ctwa)
   Wired into the PBM boilerplate; see docs/13-toefl-landing-page.md.
   - CTAs use TrackedCTA so every zone/action is measured.
   - WhatsApp links are built from the WHATSAPP_NUMBER prop.
   - Sections carry stable ids for the section-view tracker.
   Styling: Tailwind CSS v4 arbitrary properties keep the design 1:1.
   Assets live in public/assets/ and are referenced as /assets/*
   ============================================================ */

/** Fallback when routes/web.php does not provide WHATSAPP_NUMBER. */
const FALLBACK_WA_NUMBER = '6285255499299';
const waUrl = (number: string, text: string): string =>
...
const KEYFRAMES = `
  @layer base {
  /* The design source is plain HTML, where content-box is the default and only 11
     elements opt into border-box. Tailwind preflight flips that default, so restore
     it here; the explicit box-sizing utilities used by the page still win. */
  *, *::before, *::after { box-sizing: content-box; }
  button, input, select, textarea { box-sizing: border-box; }
  /* Tailwind preflight blocks images; plain HTML keeps them inline, which adds the
     same line-box space the reference has. */
  img, video, canvas, audio, iframe, embed, object { display: inline; vertical-align: baseline; }
  body { margin: 0; font-family: 'Nunito', system-ui, sans-serif; line-height: normal; }
...
type LandingPageProps = {
    /** Digits-only WhatsApp number, injected by routes/web.php from WHATSAPP_NUMBER. */
    whatsappNumber?: string;
};

export default function LandingPage({ whatsappNumber }: LandingPageProps) {
    const waNumber = (whatsappNumber || FALLBACK_WA_NUMBER).replace(/\D+/g, '');
    const waLink = (text: string): string => waUrl(waNumber, text);

    const [scrolled, setScrolled] = useState<boolean>(false);
    const [bannerH, setBannerH] = useState<number>(38);
    const [mode, setMode] = useState<PricingMode>('self');
    const [openFaqs, setOpenFaqs] = useState<number[]>([0, 6, 10]);
    const isFaqOpen = (i: number): boolean => openFaqs.includes(i);
    ...
    /** Same value as `mode`, but not narrowed by the render branches below. */
    const activeMode: PricingMode = mode;
    ...
    useEffect(() => {
        ...
        // localStorage is client-only, so the real value is synced after mount.
        // Doing it during render would break hydration (server renders 12:00:00).
    ...
    useEffect(() => {
        // URLSearchParams is client-only, so sync after mount to avoid a
        // hydration mismatch when the page is server rendered.
    ...
    useEffect(() => {
        const measure = (): void => {
            ...
            setBannerH(
                banner ? Math.round(banner.getBoundingClientRect().height) : 0,
            );
        };
        measure();
    ...
    return (
        <>
            <Head title="TOEFL ITP Full Bright Indonesia — Capai TOEFL 500+ dalam 15 Hari" />
            <style>{KEYFRAMES}</style>

            <div className="[min-height:100vh] [font-family:Nunito,system-ui,sans-serif] [line-height:normal] [background:#fff]">
                {/* Urgency Banner */}
                {flashVisible ? (
                    <>
                        <TrackedCTA
                            zone="sticky"
                            action="scroll"
                            label="🔥 FLASH SALE SEPTEMBER · DISKON 60% 🔥 FLASH SALE"
                            id="urgency-banner"
                            href="#pricing"
                ...
                >
                    <div className="[margin:0_auto] [display:flex] [height:64px] [max-width:1152px] [align-items:center] [justify-content:space-between] [padding:0_24px]">
                        ...
                        <TrackedCTA
                            zone="nav"
                            action="scroll"
                            label="🎓 Amankan Seat Rp250rb Rp99rb -60%"
                            href="#pricing"
                ...
                <div
                    id="alumni"
                    className="[overflow:hidden] [padding:32px_0] [background:#F3F3F3]"
                >
                    ...
                    <div className="[overflow:hidden] [mask-image:linear-gradient(to_right,transparent,black_6%,black_94%,transparent)] [-webkit-mask-image:linear-gradient(to_right,transparent,black_6%,black_94%,transparent)]">
                        <div className="[display:flex] [width:max-content] [animation:infiniteScroll_30s_linear_infinite]">
                            <div
                                ...
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-ui.png)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>

                            <div
                                role="img"
                                aria-label="Institut Teknologi Bandung"
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-itb.png)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>

                            <div
                                role="img"
                                aria-label="Universitas Gadjah Mada"
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-ugm.webp)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>

                            <div
                                role="img"
                                aria-label="IPB University"
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-ipb.png)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>
                            ...
                            <div
                                ...
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-unpad.svg)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>

                            <div
                                role="img"
                                aria-label="Institut Teknologi Sepuluh Nopember"
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-its.png)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>

                            <div
                                role="img"
                                aria-label="Universitas Diponegoro"
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-undip.png)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>

                            <div
                                role="img"
                                aria-label="University of Nottingham"
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-nottingham.png)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>

                            <div
                                role="img"
                                aria-label="Universität Stuttgart"
                                className="[margin:0_20px] [height:64px] [width:110px] [flex-shrink:0] [background-image:url(/assets/univ-stuttgart.png)] [background-size:contain] [background-position:center] [background-repeat:no-repeat]"
                            ></div>
                    ...
                    <div className="[margin:0_auto] [max-width:1152px]">
                        ...
                        <div className="[margin-top:48px] [margin-right:auto] [margin-left:auto] [max-width:520px]">
                            ...
                            >
                                <video
                                    ref={videoRef}
                                    src="/media/testimoni iyha.mp4#t=1.5"
                                    controls
                                    playsInline
                                    preload="metadata"
                ...
                <div className="[margin:0_auto] [max-width:1152px]">
                    ...
                    <div className="[margin:0_auto_48px] [max-width:768px] [border-radius:24px] [padding:0_28px] [box-shadow:0_4px_24px_rgba(0,0,0,0.06)] [background:#fff]">
                        <div style={css(faqItemStyle(activeCat, 0))}>
                            ...
                            >
                                <span style={css(faqQStyle(isFaqOpen(0)))}>
                                    Kalau ambil paket Self-Study LMS, apa saja
                                    yang saya dapat?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(0)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(0) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 1))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(1)))}>
                                    Bagaimana cara akses LMS setelah saya bayar?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(1)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(1) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 2))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(2)))}>
                                    Saya belajar sendiri di LMS. Kalau bingung,
                                    bisa tanya ke siapa?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(2)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(2) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 3))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(3)))}>
                                    Apakah bisa dicoba dulu sebelum bayar?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(3)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(3) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 4))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(4)))}>
                                    Apakah bisa belajar tanpa terikat jadwal
                                    karena saya sibuk?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(4)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(4) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 5))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(5)))}>
                                    Apakah metode ini cocok untuk pemula yang
                                    grammar-nya sangat lemah?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(5)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(5) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 6))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(6)))}>
                                    Kenapa belajar di sini beda dengan belajar
                                    sendiri dari buku dan YouTube?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(6)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(6) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 7))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(7)))}>
                                    Berapa kenaikan skor yang bisa saya
                                    harapkan?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(7)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(7) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 8))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(8)))}>
                                    Apakah dijamin bisa mencapai skor 500?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(8)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(8) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 9))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(9)))}>
                                    Apakah ada batasan usia untuk mengikuti
                                    program ini?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(9)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(9) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 10))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(10)))}>
                                    Apa bedanya paket Dibimbing Tutor dengan
                                    Self-Study LMS?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(10)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(10) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 11))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(11)))}>
                                    Kapan jadwal LIVE ZOOM-nya dan apakah bisa
                                    dipilih?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(11)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(11) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 12))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(12)))}>
                                    Kalau saya tidak bisa hadir LIVE ZOOM,
                                    bagaimana?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(12)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(12) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 13))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(13)))}>
                                    Apakah saya dapat sertifikat TOEFL?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(13)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(13) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 14))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(14)))}>
                                    Apakah lembaganya resmi dan sertifikatnya
                                    valid?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(14)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(14) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 15))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(15)))}>
                                    Bagaimana cara mendaftar dan metode
                                    pembayaran apa saja?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(15)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(15) ? (
                        ...
                        <div style={css(faqItemStyle(activeCat, 16))}>
                            <button
                                ...
                                className="[display:flex] [width:100%] [cursor:pointer] [align-items:flex-start] [justify-content:space-between] [gap:16px] [padding:20px_0] [text-align:left] [background:none] [border:none]"
                            >
                                <span style={css(faqQStyle(isFaqOpen(16)))}>
                                    Apakah ada garansi kalau skor saya belum
                                    mencapai target?
                                </span>
                                <span style={css(faqChevStyle(isFaqOpen(16)))}>
                                    ▾
                                </span>
                            </button>
                            {isFaqOpen(16) ? (
            ...
            <div
                id="wa-bubble"
                className="[position:fixed] [right:20px] [bottom:20px] [z-index:52] [display:flex] [flex-direction:column] [align-items:flex-end] [gap:10px]"
            >
                {waBubbleOpen ? (
                    <>
                        <div className="[position:relative] [max-width:270px] [border-radius:18px_18px_6px_18px] [padding:14px_16px_14px_14px] [box-shadow:0_10px_34px_rgba(0,0,0,0.18)] [background:#fff] [border:1px_solid_#e5e7eb] max-[559px]:[max-width:208px] max-[559px]:[border-radius:14px_14px_5px_14px] max-[559px]:[padding:10px_12px_10px_11px]">
                            ...
                            <TrackedCTA
                                zone="floating"
                                action="whatsapp"
                                label="Mr. Choiri - Admin Full Bright Masih bingung ata"
                                href={waLink(
