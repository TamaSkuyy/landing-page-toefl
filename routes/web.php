...
/*
 * Media files that need HTTP Range support (206 Partial Content).
 *
 * `php artisan serve` serves existing files from public/ by itself and ignores the Range
 * header, so browsers refuse to play or seek <video> sources. Routing the file through
 * Laravel (a path that is not a real file) makes BinaryFileResponse answer Range requests
...
})->where('file', '[A-Za-z0-9 _().-]+')->name('media');
